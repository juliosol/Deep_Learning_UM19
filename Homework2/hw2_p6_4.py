...
class WordEmbeddingAvgPooling(nn.Module):
	...
	def __init__(self, vocab_size, embedding_dim, n_out):
		...
		self.rnn = nn.RNN(embedding_dim, 20, 2, embedding_dim)
		#self.avg_pooling = nn.AvgPool1d(3, stride=1)
		self.linear = nn.Linear(embedding_dim, n_out)
		
        	
	def forward(self, x):
		h0 = Variable(torch.zeros(20, x.size(0), 2))
		out, hn = self.rnn(x)
		out = self.linear(out)
...
def embedding_avg(embedding_dim, list_words, word_to_ix, glove=False):
	...
	total = torch.zeros([1, embedding_dim], dtype=torch.double)
	#print(word_to_ix)
	for w in list_words:
		if glove is False:
			...
			#print(word_to_ix[w])
			print(w)
			if w == '_num_' or w == '\'burgh':
				pass
			else:
				curr_vector = torch.tensor(word_to_ix[w], dtype=torch.double)
				#print(curr_vector)
				embedded = curr_vector
				#embedded = embedding(curr_vector)
				total = total + embedded
...
def main():
	...
	model_bow, word_to_ix = train(train_text, train_labels, embeddings_index, model_type='bow', context_size=10, 
      embedding_dim=100, batch_size=256, num_epochs=10, lr=0.01, device=device, glove=True)
	...
	with open('predictions_q4.txt', 'w') as f:
