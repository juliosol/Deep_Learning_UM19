...
class SiameseNetwork(nn.Module):
    def __init__(self):
        ...
        self.f_net = nn.Sequential(
                    ...
                    #nn.MaxPool2d(kernel_size=2, stride=1, dilation=2, padding=1),
...
class ContrastiveLoss(torch.nn.Module):
    ...
    def forward(self, output1, output2, label):
        ...
        difference = F.pairwise_distance(output1, output2).unsqueeze(1)
        first_loss = (1-label) * torch.pow(difference, 2)
        second_loss = label*(torch.clamp(self.margin - difference, min=0.0))**2
        contrastive_loss = torch.mean(first_loss + second_loss)
...
def evaluate(dataiter, net, split, device):
    for i in range(2):
        ...
        for j in range(10):
            ...
            imshow(torchvision.utils.make_grid(concatenated),'%s, dissimilarity:%.2f'%(split, torch.mean(euclidean_distance))) 
            plt.savefig('%s_%d_%d.jpg'%(split,i, j))
            plt.close()


########## ASK about pairwise distance dimensions 
...
if __name__ == "__main__":
    ...
    train_dataloader = DataLoader(siamese_dataset, shuffle=False, num_workers=1, batch_size=1)
