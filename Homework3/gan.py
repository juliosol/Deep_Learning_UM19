...
def show_images(images, epoch):
    ...
    fig.savefig("Figures %r.png" % epoch)
...
def build_discriminator(batch_size):
    ...
    * Fully Connected size 1
    ...
    return nn.Sequential(
        ...
        ## Ask how to determine these numbers
...
def build_generator(batch_size, noise_dim):
    ...
    * Should have a 28x28x1 image, reshape back into 784 vector
    ...
    """
    #print("This is noise_dim", noise_dim)
    return nn.Sequential(
        ###########################
        ######### TO DO ###########
        ###########################
...
def bce_loss(input, target):
    ...
    loss = torch.clamp(input, min=0) - input * target + (1 + neg_abs.exp()).log()
...
def discriminator_loss(logits_real, logits_fake, dtype):
    ...
    target_real = torch.ones(logits_real.size())
    target_fake = torch.zeros(logits_fake.size())
...
def generator_loss(logits_fake, dtype):
    ...
    target_fake = torch.ones(logits_fake.size())
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            if (iter_count % show_every == 0):
                ...
                show_images(imgs_numpy[0:16], epoch)
                print(epoch)
                plt.pause(1.0)
                print()
            iter_count += 1
...
def main():
    ...
    show_images(imgs, epoch="Initial")
